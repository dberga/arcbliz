...
void Item::ApplyEnchantmentBonus(uint32 Slot, bool Apply)
{
	...
	if(VisibleBase <= PLAYER_VISIBLE_ITEM_19_ENCHANTMENT)
		...
		LOG_DETAIL("ERROR: Item::ApplyEnchantmentBonus visual out of range! Tried to address UInt32 field %i !!!", VisibleBase);
	...
	for(uint32 c = 0; c < 3; c++)
	{
		if(Entry->type[c])
		{
			...
			switch(Entry->type[c])
			{
				...
				default:
					{
						LOG_DETAIL("ERROR: Unknown enchantment type: %u (%u)", Entry->type[c], Entry->Id);
					}
					break;
			}
		}
...
uint32 Item::RepairItemCost()
{
	...
	if(dcosts == NULL)
	{
		LOG_DETAIL("ERROR: Repair: Unknown item level (%u)", dcosts);
		return 0;
	}
	...
	if(dquality == NULL)
	{
		LOG_DETAIL("ERROR: Repair: Unknown item quality (%u)", dquality);
		return 0;
	}
