...
void WorldSession::HandleUnstablePet(WorldPacket & recv_data)
{
	...
	if(!pet)
	{
		LOG_DETAIL("ERROR: PET SYSTEM: Player " I64FMT " tried to unstable non-existent pet %d", _player->GetGUID(), petnumber);
		return;
	}
	...
	for(std::list<Pet*>::iterator itr = summons.begin(); itr != summons.end(); ++itr)
	{
		if((*itr)->GetGUID() == guid)  //guid should be the pet guid
		{
			if(!(*itr)->RemoveAura(spellid))
				LOG_DETAIL("ERROR: PET SYSTEM: Player " I64FMT " failed to cancel aura %u from pet", _player->GetGUID(), spellid);
			break;
		}
	}
}
